    ...
    attach: function (context, settings) {
      ...
      jQuery('document').ready(function (e) {
        ...
        jQuery('a#config-localist-fields').click(function (e) {
          ...
          if (txt && txt === '') {
            ...
            jQuery('div#wrong-url').html('<div id="console" class="clearfix"><div class="messages error"><h2 class="element-invisible">' +
              Drupal.t('Error message') +
              '</h2>' +
              Drupal.t('Please enter the URL.') +
              '</div></div>');
            jQuery('input[name="url"]').addClass('error');
          }
          else if (!is_valid_url(txt)) {
            jQuery('div#widget-data').css('display', 'none');
            jQuery('div#wrong-url').html('<div id="console" class="clearfix"><div class="messages error"><h2 class="element-invisible">' +
              Drupal.t('Error message') +
              '</h2>' +
              Drupal.t('You have entered the wrong URL.') +
              '</div></div>');
            jQuery('input[name="url"]').addClass('error');
          }
          else {
            ...
            jQuery.ajax({
              url: settings.basePath + 'save/lurl',
              method: 'GET',
              data: 'lurl=' + txt,
              success: function (response) {
                if (response === '0') {
                  jQuery('div#wrong-url').html('<div id="console" class="clearfix"><div class="messages error"><h2 class="element-invisible">' +
                    Drupal.t('Error message') +
                    '</h2>' +
                    Drupal.t('Please enter a valid Localist URL.') +
                    '</div></div>');
                  jQuery('div#widget-data').css('display', 'none');
                }
                else {
      ...
      jQuery(window).load(function () {
        ...
        if (st === 'edit' || st === 'new') {
          ...
          jQuery('div#widget-data').css('display', 'block');
          jQuery('.wrape_container select').each(function () {
        ...
        if (st === 'edit') {
          jQuery('.filters_block input[ftype="filters"]').each(function () {
            var input = jQuery(this);
            var val = input.val();
            if (val && val !== '') {
              var name = input.attr('fname');
              jQuery('.all_filters select[fname="' + name + '"] option').each(function () {
                var opt = jQuery(this).val();
                if (val === opt) {
                  jQuery(this).prop('selected', 'selected');
          ...
          jQuery('.filters_block input[ftype="exclude_filters"]').each(function () {
            var input = jQuery(this);
            var val = input.val();
            if (val && val !== '') {
              var name = input.attr('fname');
              jQuery('.all_exclude_filters select[fname="' + name + '"] option').each(function () {
                var opt = jQuery(this).val();
                if (opt === val) {
                  jQuery(this).prop('selected', 'selected');
                  jQuery(this).attr('selected', 'selected');
      ...
      function urlgenration(txt) {
        ...
        if (is_valid_url(txt) && txt !== '') {
          ...
          jQuery('select[ftype="groups"] option:selected').each(function () {
            groups.push(jQuery(this).val());
          });
          var all_groups = groups.join('%2C');
          jQuery('select[ftype="places"] option:selected').each(function () {
          ...
          jQuery('.all_filters select').each(function () {
            var val = jQuery(this).find('option:selected').each(function () {
              if (val && val !== '') {
          ...
          jQuery('.all_exclude_filters select').each(function () {
            var val = jQuery(this).find('option:selected').each(function () {
              if (val && val !== '') {
          ...
          jQuery('#embed').val(' ').val(scriptags);
